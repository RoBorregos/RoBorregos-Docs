...
### Abstract
Our two robots, **Colibri** (Striker) and a **Ajolote** (Defender) autonomously detect an IR-emitting ball, track goals by colour, navigate within field boundaries, and coordinate offensive and defensive roles without any human intervention. The electronics are built around a Teensy 4.1 main controller connected to five custom PCBs, an omnidirectional three-motor drive base, a 15-receiver IR ring for ball tracking, and a Pixy 2.1 camera for goal detection. The codebase is fully modular, using a cosine-based motion library and cascaded moving-average filters for stable sensor readings
